...
int main(void)
{
    ...
    MiniEngine::Texture texture = engine.loadTexture(
        RESOLVE_PATH("/assets/powder.png"), MiniEngine::Texture::TextureType::Default, false);
    MiniEngine::Texture normal = engine.loadTexture(
        RESOLVE_PATH("/assets/normal.jpg"), MiniEngine::Texture::TextureType::Default, false);
    // MiniEngine::Texture roughness =
    // engine.loadTexture(RESOLVE_PATH("/assets/roughness.jpg"),
    // MiniEngine::Texture::TextureType::Default, false); MiniEngine::Texture
    // texture2 = engine.loadTexture(RESOLVE_PATH("/assets/hdri.hdr"),
    // MiniEngine::Texture::TextureType::CubeMap, true);
    ...
    // auto mesh3 = MiniEngine::Renderable::Builder()
    //	.addMaterialInstance(&material)
    //	.addBufferData(
    //		{
    //			0.0, 0.5, 1.0, 0.0, 0.0,
    ...
    // engine.addSlider("Light-X", &(light->position.x), -1, 1, [&]() {
    // scene.addLight(light); }); engine.addSlider("Light-Y",
    // &(light->position.y), -1, 1, [&]() { scene.addLight(light); });
    // engine.addSlider("Light-Z", &(light->position.z), -1, 1, [&]() {
    // scene.addLight(light); }); engine.addSlider("Light-Intensity",
    // &(light->intensity), 0, 10, [&]() { scene.addLight(light); });
    ...
    // float r =
    // material.materialProperties[(int)MiniEngine::Material::PropertyType::Roughness];
    // float g =
    // material.materialProperties[(int)MiniEngine::Material::PropertyType::Metallic];
    // engine.addSlider("Roughness", &r, 0, 1, [&]() {
    // material.materialProperties.set((int)MiniEngine::Material::PropertyType::Roughness,
    // r); }); engine.addSlider("Metallic", &g, 0, 1, [&]() {
    // material.materialProperties.set((int)MiniEngine::Material::PropertyType::Metallic,
    // g); }); engine.addCheckbox("post-process", params.enablePostProcess);
